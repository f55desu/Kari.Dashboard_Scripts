...
wb_dashboard_db.py
====================
Загрузка последнего дня из CSV-файла ДБсПризнаками.csv в PostgreSQL.

База:   analytics
Схема:  work
Таблица: work.wb_dashboard_daily

Источник CSV:
    \\kari.local\...\!!!_ИСХОДНИКИ ДЛЯ ДАШБОРДА_НЕ УДАЛЯТЬ_!!!\ДБсПризнаками.csv

Подключение к PostgreSQL — через SSH-туннель (localhost:15432).
Учётные данные читаются из config/postgres_config.json.

Модуль спроектирован так же, как wb_union_db.py: функция
...
TABLE_SCHEMA = "work"
TABLE_NAME = "wb_advert_dashboard_daily"
...
def _get_existing_dates(engine):
    from sqlalchemy import text
...
def upload_missing_days_to_postgres(config_path=None, csv_path=CSV_PATH, batch_size=200):
    ...
    """
    df = _read_full_csv(csv_path)
    csv_dates = set(df[DATE_COLUMN].dropna().dt.date)

    engine = _get_engine(config_path)
    try:
        db_dates = _get_existing_dates(engine)
        missing = sorted(csv_dates - db_dates)

        if not missing:
            _safe_print(f"✅ Все {len(csv_dates)} дат из CSV уже есть в {FULL_TABLE}, догрузка не требуется.")
            return 0

        _safe_print(f"В CSV {len(csv_dates)} дат, в SQL {len(db_dates)} дат, недостаёт {len(missing)} дат.")

        rename_map = _shorten_columns(df.columns.tolist())
        if rename_map:
            df = df.rename(columns=rename_map)
            _safe_print(f"Укорочено {len(rename_map)} колонок для PostgreSQL")

        total_rows = 0
        for i, dt in enumerate(missing, 1):
            df_day = df[df[DATE_COLUMN].dt.date == dt].copy()
            df_day[DATE_COLUMN] = df_day[DATE_COLUMN].dt.date
            with engine.begin() as conn:
                df_day.to_sql(
                    TABLE_NAME,
                    conn,
            ...
            _safe_print(f"  [{i}/{len(missing)}] {dt} — {len(df_day)} строк")

        _safe_print(f"✅ Догрузка завершена: {total_rows} строк за {len(missing)} дат в {FULL_TABLE}")
        return total_rows
    finally:
        engine.dispose()
...
def upload_full_csv_to_postgres(config_path=None, csv_path=CSV_PATH, batch_size=200):
    ...
    try:
        ...
        for i, dt in enumerate(dates, 1):
            ...
            total_rows += len(df_day)
            _safe_print(f"  [{i}/{len(dates)}] {pd.Timestamp(dt).date()} — {len(df_day)} строк")

        _safe_print(f"✅ Полная загрузка завершена: {total_rows} строк за {len(dates)} дат в {FULL_TABLE}")
        return total_rows
    finally:
        engine.dispose()
...
if __name__ == "__main__":
    ...
    group.add_argument("--full", action="store_true", help="Загрузить весь CSV целиком (DROP + INSERT)")
    group.add_argument("--sync", action="store_true", help="Догрузить только недостающие дни")
    args = parser.parse_args()

    try:
        if args.full:
            upload_full_csv_to_postgres()
        elif args.sync:
            upload_missing_days_to_postgres()
        else:
