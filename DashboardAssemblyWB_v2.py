...
def assemble():
    ...
    # [v2] Убрано искусственное переименование разделов.
    # Раздел сохраняет исходные значения "Единая Ставка" / "Ручная Ставка".
    ...
    # [v2] Маппинг «сырых» значений разделов теперь пустой — Раздел остаётся как в источнике.
    TYPE_DICT_RAW = {}

    # [v2] Приоритет при конфликте — теперь по исходным разделам.
    ...
    # [v2] Раздел остаётся в исходном виде — TYPE_DICT_RAW пустой.
    TYPE_DICT_RAW = {}
    ...
    # ----------- настройки -----------
    KEY_COLS = ['Дата', 'Артикул WB']
    # [v2] Раздел теперь "Единая Ставка"/"Ручная Ставка" → префиксы соответственно.
    ...
    def _make_right_unique(df_expenses: pd.DataFrame) -> pd.DataFrame:
        ...
        - бинарные флаги ('Единая Ставка','Ручная Ставка','Органика') берём max;
        ...
        # [v2] Флаги — теперь по исходным разделам.
                ...
                if file.endswith((".xlsx", ".xls")):
                    try:
                        ...
                        temp_df.rename(columns={
                            "data_day": "Дата",
                            "nm": "Артикул WB",
                            # [drop-DWH-metrics] Раньше из выгрузки переименовывались и тянулись далее
                            # столбцы "Показы из выгрузки" / "Клики из выгрузки" / "Корзины из выгрузки" /
                            # "Заказы из выгрузки". Они не нужны в итоговом дашборде — удалены ниже.
                        ...
                        # [drop-DWH-metrics] Оставляем только ключи мерджа, метрики из выгрузки выкидываем
    ...
    def allocate_campaign_metrics_rowwise_intsafe(
        ...
        df_item_features: pd.DataFrame,     # Дата, Артикул, Показы (или Показы, всего), <Раздел>_ID кампании
        shows_cols=('Показы, всего','Показы'),
        # [v2] Раздел теперь "Единая Ставка"/"Ручная Ставка"
    ...
    # Префиксы остаются "Единая Ставка"/"Ручная Ставка" (как в данных).
    # =====================================================================
    DASH = ' — '

    # Тело метрик при развороте по разделу
    WB_PREFIX_BODY_MAP = {
    ...
    # Префиксы (новые имена разделов)
    WB_PREFIXES = ('Единая Ставка', 'Ручная Ставка')

    # Прямой mapping для всех остальных колонок
    WB_DIRECT = {
        # Воронка
        ...
        # Справочник
        # 'Артикул' оставляем без переименования
        ...
        # 'Заказы из выгрузки':           'Количество заказов по выгрузке',
    ...
    # [v2] Тестовый запуск: НЕ обновляем боевой "Показы и затраты ВБ_2.0.xlsx".
    print("[v2] Тестовый прогон: обновление боевого Excel-файла пропущено.")
    # return  # ранний выход; ниже — старый код, оставлен закомментированным.

    # 25. Обновить файл "Показы и затраты ВБ_2.0.xlsx"
    try:  # [v2] dead code — недостижимо из-за return выше
